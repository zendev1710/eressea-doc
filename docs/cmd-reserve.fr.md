...
L'unité prend ses marchandises dans n'importe quelle unité (généralement de haut en bas selon l'ordre dans le NR), à moins que cette unité n'ait réservé cet objet (voir [réserve d'objets][reserve-d-objets-id] !).  

Avec l'ordre `RESERVE ALL <Item>`, une unité réserve tout ce qu'elle possède de l'élément spécifié.  

Avec l'ordre `RESERVE EACH <quantité> <Item>`, le nombre d'items spécifié est réservé **par personne dans l'unité**.  
...
- Si les unités d'une faction réservent plus d'un objet donné que ce qui est disponible dans la région (dans la réserve d'objets) dans son ensemble, le résultat est difficile à prédire. Pour plus de détails, voir [Réserve d'objets¶][items-pool]
...
- [Réserve d'objets][reserve-d-objets-id]
- [`GIVE`][cmd-give-fr]
