...
You control exactly what the evaluation looks like.  

The available options are the following:

- `PLAINTEXT`: This is the normal evaluation in plain text.
  If you only use the computer evaluation, the normal evaluation can be omitted
...
- `STATISTICS`: With this option, a small statistic is displayed after each region in the normal analysis
- `SCORE`: With this option, **from the 13th round at the earliest**, a score is issued that allows a small comparison with other factions
- `TEMPLATE`: A separate file contains a [[orders|template for the next round's orders]].
  This can be turned off and on again. If you don't need this, for example because you use a tool to create a move, you should turn off the move template
...
- `ADDRESSES`: This appends the address list of the factions seen in the group to the report
