...
To alleviate the phenomenon, we have the command [[cmd-quit]] with the argument FACTION, which transfers the units of the leaving faction to an allied faction, which ensures that the unit limit and the upper limits for mages, heroes, etc. are adhered to.
