...
- A unit limit is introduced; starting at 2000, it will be reduced by 200 every month until it reaches 1000.
  At the time of launch, 9 factions are over the limit
...
- Farmer smoothing is an attempt to distribute the important raw material farmers more fairly.
  ...
  The number of farmers in Eressea remained the same.
  Not only the farmers but also the players in the region were important for the distribution.
...
- The coordinates are no longer absolute but relative to the player's starting region.
  Each player now starts at the coordinates (0,0)
- Fights no longer last 10 rounds, but only 5 rounds or 6 with the tactician round
- Migrants can no longer learn expensive skill
  Units with existing expensive skills cannot be transferred
