# Règles

Ceci est le guide du jeu *Eressea*. Eressea se joue exclusivement par e-mail. Ceux qui n'ont pas d'adresse e-mail ne peuvent malheureusement pas jouer à Eressea. Eressea est la version allemande d'[Atlantis], un jeu développé en 1993 par Russell Wallace. Eressea fonctionne sans interruption depuis 1996. Dans Eressea, tu prends la direction d'une *faction* et tu joues avec de nombreuses autres factions dirigées par d'autres personnes dans le même monde fantastique.

Ces règles sont relativement complexes, mais tu devrais quand même les lire avant de commencer à jouer. Il suffit de suivre les liens *Lire la suite* en bas de chaque page pour avoir le fil conducteur. Bien sûr, tu ne dois pas te souvenir de tous les détails dès le début. Tu feras des erreurs qui auront bien sûr un impact négatif sur ta faction, mais ce n'est pas grave ! D'autres font aussi des erreurs et, avec l'aide des autres joueurs, ta faction aura quand même une chance de survivre. Eressea n'est pas un jeu rapide. Les événements se mesurent en mois et en années (en temps réel) plutôt qu'en jours ou en semaines. Néanmoins, beaucoup attendent chaque semaine avec impatience le prochain tour et certains sont là depuis des décennies.

## Mise en garde

     POUR LES JOUEURS FRANCAIS
     Les règles et explications sont en français MAIS pas le jeu. Vous devez choisir entre l'allemand (DE) et l'anglais (EN)

Eressea est un jeu vivant. Il a connu des améliorations continues au cours des dernières années et nous continuerons à travailler pour l'améliorer. Cela signifie que les règles peuvent changer. Malheureusement, de tels changements ont donné lieu à de longues disputes entre l'équipe de direction et quelques joueurs mécontents.

L'édition de ces règles est autorisée et encouragée, mais il est important de respecter les [Conseils pour l'édition des règles] !

|     |     |
| --- | --- |
| Weiterlesen: | [Introduction] |

[Introduction]: /Spezial:Meine_Sprache/Einleitung "Spezial:Meine Sprache/Einleitung"

<!-- Récupéré depuis [https://wiki.eressea.de/index.php?title=Regeln/fr&oldid=14182] -->

[Kategorie][]:

- [Regeln/fr]

  [Atlantis]: /Atlantis "Atlantis"
  [Conseils pour l'édition des règles]: /Spezial:Meine_Sprache/Eressea:Mitmachen "Spezial:Meine Sprache/Eressea:Mitmachen"
  [Kategorie]: /Spezial:Kategorien "Spezial:Kategorien"
  [Regeln/fr]: /index.php?title=Kategorie:Regeln/fr&action=edit&redlink=1 "Kategorie:Regeln/fr (Seite nicht vorhanden)"
