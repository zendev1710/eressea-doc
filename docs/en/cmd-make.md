...
**`MAKE`**`[`*`amount`*`]`*`item`*  
**`MAKE`**`[`*`amount`*`] HERBS`  
**`MAKE TEMP`**` `*`unit-alias-id`*`["`*`name`*`"]`  
**`MAKE`**`[`*`level`*`]`*`building_type`*`[`*`id_building`*`]`  
**`MAKE`**`[`*`level`*`]`*`ship_type`*  
**`MAKE`**`[`*`level`*`] SHIP [`*`ship-id`*`]`  
**`MAKE`**`[`*`level`*`] ROAD`*`direction`*` `

The `MAKE` order is the general production order. To find out more, see the [production] chapter.

## Items

[Items] and [raw materials] are created with `MAKE [`*`number`*`]`*`item`*` `. Depending on the item, a certain skill and perhaps also certain raw materials are required. Without specifying *number*, the unit will produce as many items as people, skill and possibly raw materials allow it to.

### Potions

Creating a potion is very complex and can only be done by particularly talented alchemists. Details can be found in the [Alchemy chapter].

### Herbs

Herbs are produced with the command `MAKE HERBS` by units with the skill [herbology]. For details, see the chapter on [herbs][herbology].
...
With `MAKE`*`ship_type`* a unit starts building a new [ship]. This requires the shipcraft skill and wood. It can continue building with `MAKE [`*`levels`*`] SHIP`*`ship-nr`*. Ships cannot be expanded like castles; the type is determined at the start of construction. Ships can only be built up to the size specified by their type.
...
To make it easier to travel through a region with roads and bridges, use `MAKE ROAD`*`direction`*. To build [roads], the unit needs the skill [roadwork] and [stones][Items]. In glaciers, it needs a [tunnel] beforehand, in deserts a [caravanserai] and in swamps a [dam]. One stone is used per skill point of road construction. Between 50 and 250 stones are required for each desired direction, depending on the [terrain types]. Roads only work if they are complete.
...
[Alchemy chapter]: /List_of_talents "List of talents (Seite nicht vorhanden)"
...
[roadwork]: ./skills-list.md#Straßenbau "Straßenbau"
...
[terrain types]: /Gel%C3%A4ndearten "Geländearten"
