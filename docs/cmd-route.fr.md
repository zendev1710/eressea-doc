...
Avec cet ordre, l'unité se déplace dans le monde d'Eressea de la même manière qu'avec l'ordre [`MOVE`][cmd-move-fr].  

Cependant, avec l'ordre `ROUTE`, il est possible de planifier une séquence de déplacements, de sorte qu'une unité se déplace toujours de la même façon entre deux points (ou plus), ou effectue un long trajet jusqu'à ce qu'elle atteigne sa destination.  
Tous les déplacements traités sont renvoyés en fin d'ordre `ROUTE`.  

Pour annuler prématurément un déplacement (par exemple pour des bateaux qui ne sont pas censés naviguer aussi loin qu'ils le pourraient), vous pouvez utiliser une instruction `PAUSE` (abrégé : `P`).  
