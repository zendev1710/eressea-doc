...
³ Specific features depending on the race:
...
A fighter attacks an unarmored opponent and has a 60% chance of hitting them.
However, if the defender is wearing chainmail, this chance increases to 115% x 60% = 69%.
If they are wearing a shield instead, it decreases to 85% x 60% = 51%.
If they have a broadshell shield, it drops to 45%.
Armor and shields multiply.
If they are wearing chainmail and using a shield, the final chance is 115% x 60% = 69% (chainmail), 85% x 69% = 58.65% (chainmail and shield).
The final result is always rounded down to the defender's disadvantage, meaning the defense value is rounded down.
