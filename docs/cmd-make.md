...
Except when erecting a new building you can exchange the type in `MAKE type xyz` with `CASTLE` or any other building type.  

`MAKE type` or `MAKE SHIP` without further parameters is currently continuing construction on the building or ship in which the unit is currently located.

!!! warning "Attention"
    If the unit is in a building, `MAKE lighthouse` does not start a new building, but continues building on the old one.
