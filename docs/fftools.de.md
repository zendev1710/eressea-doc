...
Die Scriptbefehle beinhalten komplette Berufe (// script Transport mode=auto), zielgerichtete Befehle über mehrere Runden hinweg (// script SailTo X,Y) oder sehr konkrete Materialflussanweisungen (// script Request 100 Stein 200). Als Kommentare in den Befehlen der Einheiten werden sie zum Eressea-Server gesendet und von diesem auch wieder zurück gesendet und stehen für den nächsten Scriptlauf zur Verfügung. Bei Bedarf könnend die Scriptbefehle überarbeitet werden und ein weiterer Scriptlauf gestaret werden, bis das gewünschte Ergebnis erreicht wird. Dabei können auch "normale" Eressea-Befehle ergänzend verwendet werden.
