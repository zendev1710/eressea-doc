...
Des exemples d'utilisation sont également disponibles sur les pages de détail des commandes.  
...
Les scripts d'ordre incluent des métiers complets (//script Transport mode=auto), des ordres ciblées sur plusieurs tours (//script SailTo X,Y) ou des instructions de flux de matière très spécifiques (//script Request 100 Stein 200).  
...
Les ordres Eressea « normauxs » peuvent également être utilisés en complément.
