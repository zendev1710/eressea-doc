...
Kann auch mit `KÄMPFE` gesetzt werden.
...
Wie `KÄMPFE HINTEN`, aber die Einheit wird schon fliehen, wenn sie noch 90% ihrer Trefferpunkte hat.
...
Einer Einheit mit `KÄMPFE HELFE NICHT` wird im Kampf nicht geholfen, weder von Einheiten der eigenen Partei noch von Verbündeten.
